...
// rendered with random colors on each component to highlight the boundaries.
// To use random colors you need to place the following code at the end of the
...
/*
		...
		for (int j = 0; j < objects.size(); j++) {
			...
			objects[j]->set_material(reflective_ptr);
		}
*/
...
// This build function should not set a material for the beveled box.
...
//	beveled_box_ptr->set_material(reflective_ptr);    // don't use this
